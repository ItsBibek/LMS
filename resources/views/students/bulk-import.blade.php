...
@section('header', 'Bulk Import Students')
@section('subheader', 'Upload Excel file to import multiple students')

@section('content')
 <div class="max-w-4xl mx-auto">
  <!-- Back to Options -->
  <div class="mb-6">
   <a href="{{ route('students.create') }}" class="inline-flex items-center text-sm text-slate-600 hover:text-slate-900">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
   ...
   <div class="lg:col-span-2">
    <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      ...
      <div class="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
       <div class="flex items-start">
        ...
        <div>
         ...
         @if (session('imported'))
          <p class="text-xs text-emerald-600 mt-1">Successfully imported {{ session('imported') }} students</p>
         @endif
         @if (session('skipped'))
     ...
     <form method="POST" action="{{ route('students.bulk-import.store') }}" enctype="multipart/form-data" class="space-y-4">
      @csrf
      ...
      <div>
       ...
       <div class="relative">
        <input type="file" name="file" accept=".xlsx,.xls" required
         class="w-full px-3 py-2 rounded-lg border-2 border-dashed border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition-colors" />
       </div>
    ...
    <div class="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-100 rounded-xl p-6 shadow-sm sticky top-6">
     ...
     <ol class="text-xs text-emerald-800 space-y-2">
      ...
      <li class="flex items-start">
       ...
       <span>Fill in the student details in the Excel file</span>
      </li>
      <li class="flex items-start">
     ...
     <div class="mt-4 pt-4 border-t border-emerald-200">
      <a href="{{ route('students.download-template') }}" class="flex items-center justify-center w-full px-4 py-2.5 bg-white border border-emerald-300 text-emerald-700 rounded-lg text-sm font-medium hover:bg-emerald-50 transition-colors">
       <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
