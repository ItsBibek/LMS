   ...
   <form method="POST" action="{{ route('books.store') }}" class="space-y-4">
    ...
    <div class="flex items-start">
     <div class="flex items-center h-5">
      <input type="checkbox" id="generate_barcode" name="generate_barcode" value="1" {{ old('generate_barcode') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-slate-100 border-slate-300 rounded focus:ring-blue-500 focus:ring-2" />
     </div>
     <div class="ml-3">
      <label for="generate_barcode" class="text-sm font-medium text-slate-700 cursor-pointer">Generate barcode after saving</label>
      <p class="text-xs text-slate-500">Automatically open the barcode page for printing or downloading</p>
     </div>
    </div>
